...
def evaluate_classifier(classifier_speed, classifier_volume, X, data, random_state=42):
    """
    segments, speed_labels, volume_labels, unique_speeds, unique_volumes: are already loaded from the data_drops.pkl file.
    :param X: scaled features, 2D numpy array of shape (n_samples, n_features)
    ...
    for train_index, test_index in tqdm(skf_speed.split(X, speed_labels),
                                        total=skf_speed.get_n_splits(X, speed_labels)):
    ...
    for train_index, test_index in tqdm(skf_volume.split(X, volume_labels),
                                        total=skf_volume.get_n_splits(X, volume_labels)):
        X_train, X_test = X[train_index], X[test_index]
    ...
    print(f'Volume accuracy average across all folds: '
          f'{np.mean(all_cv_scores_volume):.4f}±{np.std(all_cv_scores_volume):.4f}'
          f' ({np.mean(all_cv_scores_volume):.3f}±{np.std(all_cv_scores_volume):.3f})')
